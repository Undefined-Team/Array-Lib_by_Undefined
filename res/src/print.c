...
static char     *ud_arr_print_get_total_arr(ud_arr *arr, char *total_arr, char *format, size_t format_len, size_t space)
{
    ud_ut_count len = arr->len;
    if (len != 0)
        return total_arr;
    ...
    else
    {
        ...
        ud_arr_print_str_add(&total_arr, UD_UT_COLOR_N);
        ud_arr_print_str_add(&total_arr, "\n");
        while (len-- > 0) total_arr = ud_arr_print_get_total_arr(arr, total_arr, format, format_len, space + 1);
        for (ud_ut_count i = 0; i < space * UD_UT_SPACE_NBR; i++) ud_arr_print_str_add(&total_arr, " ");
        ud_arr_print_str_add(&total_arr, ud_ut_color_t[space % UD_UT_COLOR_NBR]);
        ud_arr_print_str_add(&total_arr, "]");
        ud_arr_print_str_add(&total_arr, UD_UT_COLOR_N);
...
size_t          ud_arr_print_get_save_len(ud_arr *arr, size_t *norm_type_size)
{
    ...
    else if (arr->type_s != 0)
    {
        ...
        else if (*norm_type_size != arr->type_s) ud_ut_error("All sub array must have the same type or type_size");
        return arr->len;
    }
    ud_ut_count count = 0;
...
char            **ud_arr_print_get_arr(ud_arr *arr, char *format)
{
    ...
    for (size_t i = 0; i < save_len; ++i) printf("%zd ", index_cut[i]);
    printf("\nts = %zd\n", total_size);

    UD_UT_PROT_MALLOC(total_arr = ud_ut_malloc(sizeof(char) * total_size));
    (void)ud_arr_print_get_total_arr(arr, total_arr, format, format_len, 0);
