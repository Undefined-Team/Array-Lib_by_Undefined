...
# define ud_arr_set(type, ...) ({ ud_arr *new_arr; type in_val[] = {__VA_ARGS__}; type *in_tmp = in_val; size_t len = sizeof(in_val) / sizeof(type); new_arr = ud_arr_init(!ud_str_cmp(#type, "ud_arr*") ? 0 : sizeof(type), len); type *val = (type*)new_arr->val; for (ud_ut_count i = 0; i < len; ++i, ++val, ++in_val) *val = *in_tmp; new_arr; })
/*
# define ud_arr_set(type, ...) \
    ({ \
        ...
        for (ud_ut_count i = 0; i < len; ++i) *val++ = *in_val++; \
        new_arr; \
    })
