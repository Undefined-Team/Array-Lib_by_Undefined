...
# define ud_arr_set_ctr(type, len, ...) ({ size_t len = UD_ARGS_LEN(type, __VA_ARGS__); ud_arr *new_arr; new_arr = ud_arr_init(sizeof(type), len); type *val = (type*)new_arr->val; type in_val[len] = {__VA_ARGS__}; for (ud_ut_count i = 0; i < len; ++i) val[i] = in_val[i]; new_arr; })
# define ud_arr_set(type, ...)          ud_arr_set_ctr(type, UD_ARGS_LEN(type, __VA_ARGS__), __VA_ARGS__)
...
# define ud_arr_print(arr, type, format) \
    ({ \
        ...
        for (ud_ut_count i = 0; i < flattened->len; ++i, ++val) printf(ud_arr_print_arr[i], *val); \
        ud_arr_rfree(flattened); \
